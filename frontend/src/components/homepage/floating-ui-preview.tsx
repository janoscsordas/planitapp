import { IconBug, IconCheck, IconDotsVertical, IconHash, IconPlus, IconSend, IconUsers } from "@tabler/icons-react";
      ...
      <div className="p-4 space-y-3">
        {messages.map((m, i) => (
          <div key={i} className="flex items-start gap-2.5">
            <div className={`w-6 h-6 rounded-full ${m.color} flex items-center justify-center text-[10px] font-bold text-white shrink-0 mt-0.5`}>{m.avatar}</div>
...
/* ─── Mock project card ─── */
function ProjectCard({ name, members, tasks, bugs, color }: { name: string; members: number; tasks: number; bugs: number; color: string }) {
  return (
    <div className="rounded-xl border border-zinc-800/60 bg-zinc-950/60 backdrop-blur-sm p-4 hover:border-green-800/50 transition-colors group">
      ...
      <div className="mt-3 h-1 rounded-full bg-zinc-800 overflow-hidden">
        <div className={`h-full rounded-full bg-linear-to-r from-green-600 to-green-400`} style={{ width: `${(tasks / 20) * 100}%` }} />
      </div>
    </div>
  );
}
        ...
        <div className="animate-slide-up delay-500 mt-16 w-full max-w-5xl mx-auto relative">
          ...
          <div className="relative rounded-2xl border border-zinc-800/60 bg-zinc-950/80 backdrop-blur-sm overflow-hidden shadow-2xl shadow-black/60">
            ...
            <div className="flex items-center gap-2 px-4 py-3 border-b border-zinc-800/60 bg-zinc-900/40">
              ...
              <span className="ml-3 text-xs text-zinc-500 font-mono">planitapp.hu/projects/acme-corp</span>
            </div>
